...
unsigned long __stdcall SizeofMin5Byte(void *code)
{
	uint32_t  Length;
	uint8_t*  pOpcode;
	uint32_t  Result = 0;
	...
	do
	{
		Length = ldasm((uint8_t*)code, &data, is_x64);

		pOpcode = (uint8_t*)code + data.opcd_offset;
		Result += Length;
		if (Result >= 5)
			break;
...
//int main()
//{
//	char code[] = {0x55,0x8b,0xec};
//	auto l= SizeOfProc(code);
//}
